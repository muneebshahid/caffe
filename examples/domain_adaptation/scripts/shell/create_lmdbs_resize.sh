...
then
	...
	rm -r $target1
	rm -r $target2
	convert_imageset -resize_height 256 -resize_width 256 $images $images"target.txt" $target1
	convert_imageset -resize_height 256 -resize_width 256 $images $images"target.txt" $target2
elif [ "$1" = "test" ]
then
	test1=$lmdb"test1"
	test2=$lmdb"test2"
